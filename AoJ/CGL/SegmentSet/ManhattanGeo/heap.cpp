...
struct Segment
{
  public:
    ...
    static bool ascendX(Segment seg1, Segment seg2)
    {
      //@commnet If y-coordinates of the start points are the same,
      //sort by the high y-coordinate of the end points
      if(seg1.s.x == seg2.s.x){return seg1.e.x > seg2.e.x;}
...
void check_heap(vector<Segment> h)
{
  make_heap(h.begin(), h.end(), Segment::ascendX);
  int s = h.size();
  for(int i = 0; i < s; ++i){
    cout<<h.front()<<endl;
    pop_heap(h.begin(), h.end(), Segment::ascendX);
    h.pop_back();
  }
}
...
int segmentIntersection(vector<Segment>& v, vector<Segment>& h)
{
  ...
  int nIntersec = 0;
  //@comment less x-point of start point
  ...
  for(int i = 0; i < v.size(); ++i){
    ...
    Segment curSeg = tmp.front();
    // cout<<curSeg<<endl;
    while(curSeg.s.x <= v[i].s.x && j < h.size()){
      //@comment search max heap
      if(intersectLL(curSeg.s, curSeg.e, v[i].s, v[i].e)){
        nIntersec++;
      }

      pop_heap(tmp.begin(), tmp.end(), Segment::ascendX);
      tmp.pop_back();
      curSeg = tmp.front();
      ++j;
    }
    ...
    //@comment remove horizontal seg after current vertical seg
    while(h.front().e.x < v[i].s.x){
      pop_heap(h.begin(), h.end(), Segment::ascendX);
      h.pop_back();
    }
  }
...
int main(int argc, char* argv[])
{
  ...
  for(int i = 0; i < n; i++){
    ...
    if(x1 == x2 && y1 == y2){
      ...
      if(y1 > y2)
        v_segments.push_back(Segment(x2, y2, x1, y1));
      else
